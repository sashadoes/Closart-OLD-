...
class MarketingModel {
	...
	private function getStatic(){
		...
		$ini = parse_ini_file('langs/'.$lang.'/store/admin/marketing.ini', true);
	...
	private function getDynamic(){
		...
		$result = $this->DataBase->demand('eF', "SELECT name, url_name, credits FROM ".Config::DB_PREFIX."stores WHERE url_name = ? ", array($this->store));
