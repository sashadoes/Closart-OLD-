...
<content  offset_exclusive="<?php echo $dynamic['offset']['exclusive']; ?>"  offset_woman="<?php echo $dynamic['offset']['woman']; ?>" offset_man="<?php echo $dynamic['offset']['man']; ?>" offset_arts="<?php echo $dynamic['offset']['arts']; ?>" query="<?php echo trim($_GET['s']); ?>">
<items>
<?php if(!$dynamic['search']){ ?>	
 <no_items><?php echo $static['content']['NO_ITEMS']; ?></no_items> 
<?php }else{ foreach( $dynamic['search'] as $item){ ?>

 <item> 
 
 <?php if($item['type'] == 'store'){ ?>
 
<a href="<?php echo '/'.$item['url_name']; ?>">    
    <photo>
      <img src="<?php echo '/../files/stores/'.$item['url_name'].'/bio_img/bio_mid.jpg'; ?>" alt="<?php echo $item['name']; ?>">
    </photo>
  <stats>
 ...
 <a href="<?php echo '/'.$item['store'].'/item/'.$item['art']; ?>">    
    <photo>
      <img src="<?php echo '/../files/stores/'.$item['store'].'/items_img_mid/'.$item['art'].'/'.$item['art'].'_p_1.jpg'; ?>" alt="<?php echo $item['name']; ?>">
    </photo>
  <stats>
        <name><?php echo $item['name'];?></name>
          <by></by> 
...
$(window).scroll(function() {
    if($(window).scrollTop() >= $(document).height() - ($(window).height()*1.5)) {
       ...
       if($('scroll_loader').attr('loading') == 'true'){
        ...
        var offset_exclusive = Number($('content').attr('offset_exclusive'));
        var offset_woman = Number($('content').attr('offset_woman'));
        var offset_man = Number($('content').attr('offset_man'));
        var offset_arts = Number($('content').attr('offset_arts'));
      
        
        if(((offset_exclusive%5) != 0) && ((offset_woman%5) != 0) && ((offset_man%5) != 0) && ((offset_arts%5) != 0)){
        ...
        var query = $('content').attr('query');
        
        var data = 'what=scroll_load_items&offset_exclusive='+offset_exclusive+'&offset_woman='+offset_woman+'&offset_man='+offset_man+'&offset_arts='+offset_arts+'&query='+query;
                                ...
                                success: function(data){ 
                                    ...
                                    if (data['error']){ 
                                         ...
                                         */
                                              ...
                                              $('content').attr('offset_exclusive', ++offset_exclusive);
                                              $('content').attr('offset_man', ++offset_man);
                                              $('content').attr('offset_woman', ++offset_woman);
                                              $('content').attr('offset_arts', ++offset_arts);
                                             
                                         }
                                             
                                           
                                              $('content').attr('offset_exclusive', offset_exclusive + data['offset']['exclusive']);
                                              $('content').attr('offset_man', ++offset_man + data['offset']['man']);
                                              $('content').attr('offset_woman', ++offset_woman + data['offset']['woman']);
                                              $('content').attr('offset_arts', ++offset_arts + data['offset']['arts']);
